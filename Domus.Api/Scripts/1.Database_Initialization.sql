...
IF EXISTS(SELECT name FROM master.sys.sysdatabases WHERE name = 'Domus')
    BEGIN
        ALTER DATABASE Domus SET OFFLINE WITH ROLLBACK IMMEDIATE
        ALTER DATABASE Domus SET ONLINE
        DROP DATABASE Domus
    END
GO
